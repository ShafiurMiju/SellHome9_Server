...
// async function reduceUserCredit(userId, amount) {
//   // Assuming a global or externally defined database connection
...
//Skiptrace API
app.post("/api/skiptrace-swap", async (req, res) => {
  let connection;
  ...
  try {
    ...
    if (existingRows.length > 0) {
      ...
      return res.status(201).json({
        success: true,
        message: "Skiptrace record found in DB.",
...
app.post("/api/property-detail", async (req, res) => {
  try {
    ...
    const [existingRows] = await connection.execute(
      "SELECT * FROM userActions WHERE userId = ? AND propertyDetails = ?",
      [requestBody.id, requestBody.fulladdress.address]
    );
    ...
    if (existingRows.length > 0) {
      ...
      console.log("Real Estate API Response:", propertyDetailResponse.data);
      console.log("existing row", existingRows);

      // Send success response with API data
      return res.status(201).json({
        success: true,
        message: "Property from db",
    ...
    // If skiptrace doesn't exist, insert a new record in `userActions` table
    const [insertResult] = await connection.execute(
      "INSERT INTO userActions (userId, propertyDetails, date) VALUES (?, ?, ?)",
      [requestBody.id, requestBody.fulladdress.address, new Date()]
    );
